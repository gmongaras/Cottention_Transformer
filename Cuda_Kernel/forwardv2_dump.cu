...
__global__ void forward_kernel(
    ...
    // extern __shared__ __align__(sizeof(T)) unsigned char shared_memory_uchar[];T *shared_memory_K = reinterpret_cast<T *>(shared_memory_uchar);
    // T* shared_memory_Q = &shared_memory_K[D];


    // Allocate cumsum array. This will store values for V[*, d_v] * K[*, d_k] for all d_k
    T* CumsumArr = new T[D];
    // Initialize the cumsum array to 0
    ...
    for (int s = 0; s < S; s++) {
        ...
        // for (int s = 0; s < S; s++) {
        //     shared_memory_K[s * D + d_v] = K[((n * H + h) * S + s) * D + d_v];
        //     shared_memory_Q[s * D + d_v] = Q[((n * H + h) * S + s) * D + d_v];
        // }
        // __syncthreads();

        // Iterate over the entire dimension
        #pragma unroll
        for (int d_k = 0; d_k < D; d_k+=4) {
            // Compute the sum and store it in the cumsum array
            // V[s, d_v] * K[s, d_k]
            CumsumArr[d_k] += V[((n * H + h) * S + s) * D + d_v] * K[((n * H + h) * S + s) * D + d_k];
            CumsumArr[d_k+1] += V[((n * H + h) * S + s) * D + d_v] * K[((n * H + h) * S + s) * D + d_k+1];
            CumsumArr[d_k+2] += V[((n * H + h) * S + s) * D + d_v] * K[((n * H + h) * S + s) * D + d_k+2];
            CumsumArr[d_k+3] += V[((n * H + h) * S + s) * D + d_v] * K[((n * H + h) * S + s) * D + d_k+3];
...
__global__ void forward_kernel2(
    ...
    // T *shared_memory = reinterpret_cast<T *>(shared_memory_uchar);
    extern __shared__ __align__(sizeof(T)) unsigned char shared_memory_uchar[];T *shared_memory_K = reinterpret_cast<T *>(shared_memory_uchar);
    T* shared_memory_Q = &shared_memory_K[S];
    ...
    for (int d_k = 0; d_k < D; d_k++) {
        for (int s = d_v; s < S; s+=D) {
            shared_memory_K[s] = K[((n * H + h) * S + s) * D + d_k];
            shared_memory_Q[s] = Q[((n * H + h) * S + s) * D + d_k];
        }
        __syncthreads();

        T sum = 0;
        for (int s = 0; s < S; s++) {
            sum += V[((n * H + h) * S + s) * D + d_v] * shared_memory_K[s];
            output[((n * H + h) * S + s) * D + d_v] += shared_memory_Q[s] * sum;
        }
    }
...
void forward_call_double_over_d__v_cache(
    ...
    dim3 grid(N, H);
    dim3 block(D);
    forward_kernel<T><<<grid, block, 2*D*sizeof(T), stream>>>(Q, K, V, output, N, H, S, D, block_size);

    // dim3 grid(N, H);
    // dim3 block(D);
    // forward_kernel2<T><<<grid, block, 2*S*sizeof(T), stream>>>(Q, K, V, output, N, H, S, D, block_size);
}
...
__global__ void forward_kernel3(
    const T* Q, const T* K, const T* V,
    T* output,
    int N, int H, int S, const int D) {
...
// Wrapper function to orchestrate the computation
template<typename T>
void forward_call_double_over_d__v_cache3(
    const T* Q, const T* K, const T* V, T* output,
    int N, int H, int S, int D,
    const int block_size,
    cudaStream_t stream = 0) {
    ...
    // Shared memory is the cumsum and cached values
    // forward_kernel3<T><<<grid, block, D*sizeof(T) + S*sizeof(T), stream>>>(Q, K, V, output, N, H, S, D);
    forward_kernel3<T, warp_size><<<grid, block, D*sizeof(T), stream>>>(Q, K, V, output, N, H, S, D);
}
...
torch::Tensor forward_(torch::Tensor& Q, torch::Tensor& K, torch::Tensor& V, const int8_t block_size, bool inplace = false) {
    ...
    AT_DISPATCH_FLOATING_TYPES_AND_HALF(Q.scalar_type(), "forward_cuda", ([&] {
        forward_call_double_over_d__v_cache3<scalar_t>(
            Q.data_ptr<scalar_t>(),
            K.data_ptr<scalar_t>(),
            V.data_ptr<scalar_t>(),
