...
template<typename T>
__global__ void forward_kernel_quad_over_d__v_cache(
    const T* Q, const T* K, const T* V,
    T* output,
    int N, int H, int S, int d_V, int d_K,
    const int block_size) {
    ...
    int h = blockIdx.z; // Head index
    int d_v = blockIdx.x; // Dimension index within d_V
    int d_k = threadIdx.x*4; // Dimension index within d_k
    ...
    // Initialize the shared memory to 0
    if (d_k < d_K) {
        shared_memory_cumsum[d_k] = shared_memory_cumsum[d_k + 1] = shared_memory_cumsum[d_k + 2] = shared_memory_cumsum[d_k + 3] = 0;
    }
    ...
    for (int s = d_k; s < S; s += d_K) {
        ...
        if (s + 1 < S)
            shared_memory_VCache[s + 1] = V[((n * H + h) * S + s + 1) * d_V + d_v];
        if (s + 2 < S)
            shared_memory_VCache[s + 2] = V[((n * H + h) * S + s + 2) * d_V + d_v];
        if (s + 3 < S)
            shared_memory_VCache[s + 3] = V[((n * H + h) * S + s + 3) * d_V + d_v];
    }
    __syncthreads();
    ...
    for (int s = 0; s < S; s++) {
        ...
        if (d_k < d_K) {
            ...
            shared_memory_cumsum[d_k] += v * K[((n * H + h) * S + s) * d_K + d_k];
            shared_memory_cumsum[d_k + 1] += v * K[((n * H + h) * S + s) * d_K + d_k + 1];
            shared_memory_cumsum[d_k + 2] += v * K[((n * H + h) * S + s) * d_K + d_k + 2];
            shared_memory_cumsum[d_k + 3] += v * K[((n * H + h) * S + s) * d_K + d_k + 3];
            __syncthreads();

            // 2: Multiply shared[d_k] by Q[:, :, s, d_k] and store it in the second half of the shared memory shared[d_K + d_k]
            shared_memory_reduce[d_k] = shared_memory_cumsum[d_k] * Q[((n * H + h) * S + s) * d_K + d_k];
            shared_memory_reduce[d_k + 1] = shared_memory_cumsum[d_k + 1] * Q[((n * H + h) * S + s) * d_K + d_k + 1];
            shared_memory_reduce[d_k + 2] = shared_memory_cumsum[d_k + 2] * Q[((n * H + h) * S + s) * d_K + d_k + 2];
            shared_memory_reduce[d_k + 3] = shared_memory_cumsum[d_k + 3] * Q[((n * H + h) * S + s) * d_K + d_k + 3];
        }
...
// Wrapper function to orchestrate the computation
template<typename T>
void forward_call_quad_over_d__v_cache(
    const T* Q, const T* K, const T* V, T* output,
    int N, int H, int S, int D,
    const int block_size,
    cudaStream_t stream = 0) {
    ...
    // dim3 block(max_threads_per_block);
    dim3 grid(d_V, N, H);
    dim3 block((int)d_K/4);
    forward_kernel_quad_over_d__v_cache<T><<<grid, block, 2*d_K*sizeof(T) + S*sizeof(T), stream>>>(Q, K, V, output, N, H, S, d_V, d_K, block_size);
}
...
template<typename T>
__global__ void forward_kernel_eight_over_d__v_cache(
    const T* Q, const T* K, const T* V,
    T* output,
    int N, int H, int S, int d_V, int d_K,
    const int block_size) {
    ...
    int h = blockIdx.z; // Head index
    int d_v = blockIdx.x; // Dimension index within d_V
    int d_k = threadIdx.x*8; // Dimension index within d_k
    ...
    // Initialize the shared memory to 0
    if (d_k < d_K) {
        shared_memory_cumsum[d_k] = shared_memory_cumsum[d_k + 1] = shared_memory_cumsum[d_k + 2] = shared_memory_cumsum[d_k + 3] = shared_memory_cumsum[d_k + 4] = shared_memory_cumsum[d_k + 5] = shared_memory_cumsum[d_k + 6] = shared_memory_cumsum[d_k + 7] = 0;
    }
    ...
    for (int s = d_k; s < S; s += d_K) {
        ...
        if (s + 3 < S)
            shared_memory_VCache[s + 3] = V[((n * H + h) * S + s + 3) * d_V + d_v];
        if (s + 4 < S)
            shared_memory_VCache[s + 4] = V[((n * H + h) * S + s + 4) * d_V + d_v];
        if (s + 5 < S)
            shared_memory_VCache[s + 5] = V[((n * H + h) * S + s + 5) * d_V + d_v];
        if (s + 6 < S)
            shared_memory_VCache[s + 6] = V[((n * H + h) * S + s + 6) * d_V + d_v];
        if (s + 7 < S)
            shared_memory_VCache[s + 7] = V[((n * H + h) * S + s + 7) * d_V + d_v];
    }
    __syncthreads();
    ...
    for (int s = 0; s < S; s++) {
        ...
        if (d_k < d_K) {
            ...
            shared_memory_cumsum[d_k + 2] += v * K[((n * H + h) * S + s) * d_K + d_k + 2];
            shared_memory_cumsum[d_k + 3] += v * K[((n * H + h) * S + s) * d_K + d_k + 3];
            shared_memory_cumsum[d_k + 4] += v * K[((n * H + h) * S + s) * d_K + d_k + 4];
            shared_memory_cumsum[d_k + 5] += v * K[((n * H + h) * S + s) * d_K + d_k + 5];
            shared_memory_cumsum[d_k + 6] += v * K[((n * H + h) * S + s) * d_K + d_k + 6];
            shared_memory_cumsum[d_k + 7] += v * K[((n * H + h) * S + s) * d_K + d_k + 7];
            __syncthreads();
            ...
            shared_memory_reduce[d_k + 2] = shared_memory_cumsum[d_k + 2] * Q[((n * H + h) * S + s) * d_K + d_k + 2];
            shared_memory_reduce[d_k + 3] = shared_memory_cumsum[d_k + 3] * Q[((n * H + h) * S + s) * d_K + d_k + 3];
            shared_memory_reduce[d_k + 4] = shared_memory_cumsum[d_k + 4] * Q[((n * H + h) * S + s) * d_K + d_k + 4];
            shared_memory_reduce[d_k + 5] = shared_memory_cumsum[d_k + 5] * Q[((n * H + h) * S + s) * d_K + d_k + 5];
            shared_memory_reduce[d_k + 6] = shared_memory_cumsum[d_k + 6] * Q[((n * H + h) * S + s) * d_K + d_k + 6];
            shared_memory_reduce[d_k + 7] = shared_memory_cumsum[d_k + 7] * Q[((n * H + h) * S + s) * d_K + d_k + 7];
        }
        ...
        T tmp = shared_memory_reduce[d_k] + shared_memory_reduce[d_k + 1] + shared_memory_reduce[d_k + 2] + shared_memory_reduce[d_k + 3] + shared_memory_reduce[d_k + 4] + shared_memory_reduce[d_k + 5] + shared_memory_reduce[d_k + 6] + shared_memory_reduce[d_k + 7];
        tmp = blockReduce_<T>(tmp);
        if (d_k == 0) {
            output[((n * H + h) * S + s) * d_V + d_v] = tmp;
        }
...
// Wrapper function to orchestrate the computation
template<typename T>
void forward_call_eight_over_d__v_cache(
    const T* Q, const T* K, const T* V, T* output,
    int N, int H, int S, int D,
    const int block_size,
    cudaStream_t stream = 0) {
    ...
    // dim3 block(max_threads_per_block);
    dim3 grid(d_V, N, H);
    dim3 block((int)d_K/8);
    forward_kernel_eight_over_d__v_cache<T><<<grid, block, 2*d_K*sizeof(T) + S*sizeof(T), stream>>>(Q, K, V, output, N, H, S, d_V, d_K, block_size);
}
