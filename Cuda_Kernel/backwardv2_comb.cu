...
__global__ void backward_kernel(
    ...
    extern __shared__ __align__(sizeof(T)) unsigned char shared_memory_uchar[];T *CumsumArr_V = reinterpret_cast<T *>(shared_memory_uchar);
    memset(CumsumArr_V, 0, 3*D*sizeof(T));
    T* CumsumArr_G = &CumsumArr_V[D];
    T* CumsumArr_Q = &CumsumArr_V[2*D];
    ...
    for (int s = 0; s < S; s++) {
        ...
        // Initialize the sum over the inner dimension over V, G and Q to be zero
        T sum_over_V = 0;
        T sum_over_G = 0;
        T sum_over_Q = 0;

        // Load in the value of K, G and Q of the outer dimension for this part of the sequence
        T K_cache = K[((n * H + h) * S + s) * D + d_outer];
        T G_cache = G[((n * H + h) * S + s_rev) * D + d_outer];
        T Q_cache = Q[((n * H + h) * S + s_rev) * D + d_outer];

        // Iterate over the entire block of the dimension that this thread is working on
        #pragma unroll
        for (int d_inner = d_inner_start; d_inner < d_inner_end; d_inner+=2) {
            // Compute the sum and store it in the cumsum array
            // K[s, d_outer] * V[s, d_inner]
            CumsumArr_V[d_inner] += K_cache * V[((n * H + h) * S + s) * D + d_inner];
            CumsumArr_V[d_inner+1] += K_cache * V[((n * H + h) * S + s) * D + d_inner+1];
            // G[s, d_outer] * Q[s, d_inner]
            CumsumArr_G[d_inner] += G[((n * H + h) * S + s_rev) * D + d_inner] * Q_cache;
            CumsumArr_G[d_inner+1] += G[((n * H + h) * S + s_rev) * D + d_inner+1] * Q_cache;
            // G[s, d_inner] * Q[s, d_outer]
            CumsumArr_Q[d_inner] += G_cache * Q[((n * H + h) * S + s_rev) * D + d_inner];
            CumsumArr_Q[d_inner+1] += G_cache * Q[((n * H + h) * S + s_rev) * D + d_inner+1];

            // Multiply the cumsum value with the value or keys and add it to the sum
            // G[s, d_inner] * (K[s, d_outer] * V[s, d_inner])
            sum_over_V += CumsumArr_V[d_inner] * G[((n * H + h) * S + s) * D + d_inner]
                    + CumsumArr_V[d_inner+1] * G[((n * H + h) * S + s) * D + d_inner+1];
            // V[s, d_inner] * (G[s, d_inner] * Q[s, d_outer])
            sum_over_G += CumsumArr_G[d_inner] * V[((n * H + h) * S + s_rev) * D + d_inner]
                    + CumsumArr_G[d_inner+1] * V[((n * H + h) * S + s_rev) * D + d_inner+1];
            //  K[s, d_inner] * (G[s, d_outer] * Q[s, d_inner])
            sum_over_Q += CumsumArr_Q[d_inner] * K[((n * H + h) * S + s_rev) * D + d_inner]
                    + CumsumArr_Q[d_inner+1] * K[((n * H + h) * S + s_rev) * D + d_inner+1];
        }
        ...
        for (int offset = (int)warp_size/2; offset > 0; offset /= 2) {
            sum_over_V += __shfl_down_sync_(0xFFFFFFFF, sum_over_V, offset);
        ...
        if (threadIdx.x == 0) {
            Q_grad[((n * H + h) * S + s) * D + d_outer] = sum_over_V;
            K_grad[((n * H + h) * S + s_rev) * D + d_outer] = sum_over_G;
            V_grad[((n * H + h) * S + s_rev) * D + d_outer] = sum_over_Q;
        }
        __syncthreads();
    }
...
void backward_call(
    ...
    // Shared memory is the cumsum over K (Q grad), G (K grad), and Q (G grad)
    backward_kernel<T, warp_size><<<grid, block, 3*D*sizeof(T), stream>>>(Q, K, V, previous_grad, Q_grad, K_grad, V_grad, N, H, S, D);
}
...
std::vector<torch::Tensor> backward_(torch::Tensor& Q, torch::Tensor& K, torch::Tensor& V, torch::Tensor& prev_grad, const int8_t block_size) {
    ...
    auto Q_grad = torch::empty({N, H, S, D}, torch::TensorOptions().dtype(data_type).device(Q.device()));
