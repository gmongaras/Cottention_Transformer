...
template<typename T>
__inline__ __device__ void forward_kernel_quad_over_d__v_cache_one_call(
    const T* Q, const T* K, const T* V,
    T* output,
    T* shared_memory_cumsum, T* shared_memory_reduce, T* shared_memory_VCache,
    int s, int S, int n, int N, int h, int H, int d_k, int d_K, int d_v, int d_V ) {
    ...
    T tmp = shared_memory_reduce[d_k] + shared_memory_reduce[d_k + 1] + shared_memory_reduce[d_k + 2] + shared_memory_reduce[d_k + 3];
    tmp = blockReduce_<T>(tmp);
    if (d_k == 0) {
        output[((n * H + h) * S + s) * d_V + d_v] = tmp;
    }
...
template<typename T>
__inline__ __device__ void forward_kernel_quad_over_d__v_cache_loop_inner(
    const T* Q, const T* K, const T* V,
    T* output,
    T* shared_memory_cumsum, T* shared_memory_reduce, T* shared_memory_VCache,
    int s_start, int S, int n, int N, int h, int H, int d_k, int d_K, int d_v, int d_V ) {
    #pragma unroll
    for (int i = 0; i < 8; i++) {
        forward_kernel_quad_over_d__v_cache_one_call<T>(
            Q, K, V, output, shared_memory_cumsum, shared_memory_reduce, shared_memory_VCache, s_start+i, S, n, N, h, H, d_k, d_K, d_v, d_V);
    }
    // forward_kernel_double_over_d__v_cache_one_call<T>(
    //         Q, K, V, output, shared_memory_cumsum, shared_memory_reduce, shared_memory_VCache, s_start, S, n, N, h, H, d_k, d_K, d_v, d_V);
...
__global__ void forward_kernel_quad_over_d__v_cache(
    ...
    // Iterate over the entire sequence
    for (int s = 0; s < floor((float)S/8.0); s++) {
        forward_kernel_quad_over_d__v_cache_loop_inner<T>(
            Q, K, V, output, shared_memory_cumsum, shared_memory_reduce, shared_memory_VCache, s*8, S, n, N, h, H, d_k, d_K, d_v, d_V);
    }
