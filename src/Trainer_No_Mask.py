...
class Trainer_No_Mask():
    ...
    def train(self, epoch_ckpt=None, step_ckpt=None):
        ...
        for epoch in range(epoch_ckpt, 10000):
            ...
            for batch_num, batch_ in enumerate(tqdm(self.dataloader)):
                ...
                with torch.no_grad():
                    ...
                    batch_["input_ids"] = torch.stack(batch_["input_ids"]).T
                    batch_["labels"] = torch.stack(batch_["labels"]).T
                    batch_["attention_mask"] = torch.stack(batch_["attention_mask"]).T
                    
                # Loss over the sequence
                sequence_loss = 0
                
                # Lengths of each sequence in the batch
                lengths = batch_["attention_mask"].sum(-1)-1
                
                # Sample subsets of lengths
                num_samples = 10
                lengths_sample = torch.randint(1, max(lengths), (num_samples,))
                
                # Iterate over all subsets of the batch
                for length in lengths_sample:
                    # Subset ids
                    subset_ids = lengths >= length
                    
                    if subset_ids.sum() == 0:
                        break
                    
                    # Subset the batch and only keep sequences which have not ended
                    batch = {
                        "input_ids": batch_["input_ids"][:, :length].clone().detach()[subset_ids],
                        "labels": batch_["labels"][:, :length].clone().detach()[subset_ids],
                        "attention_mask": batch_["attention_mask"][:, :length].clone().detach()[subset_ids]
                    }
                        
                    with torch.autocast(device_type='cuda', dtype=torch.bfloat16) if self.use_amp else nullcontext():
                        # Send through model
                        output = self.model(batch, return_last=True)
                        
                        # Calculate loss for the last token
                        loss = loss_fn(output, batch["labels"][:, -1].to(output.device))
                        
                        # Divide loss by accumulation steps and sequence length
                        # loss = (loss / self.accumulation_steps / lengths[subset_ids].to(output.device)).mean()
                        loss = loss.mean() / self.accumulation_steps / num_samples
                ...
                loss = torch.tensor(sequence_loss)
